...
import { contentEn, contentAr, links } from './data';
...
const SocialIcons = ({ className = '' }) => (
  <div className={`flex gap-4 ${className}`}>
    <a href={links.linkedin} target="_blank" rel="noreferrer" aria-label="LinkedIn Profile" className="w-10 h-10 rounded-full flex items-center justify-center border border-gray-600 text-gray-400 hover:bg-emerald-500 hover:text-gray-900 hover:border-emerald-500 transition duration-300 transform hover:scale-110">
      <LinkedInIcon />
    </a>
    <a href={links.github} target="_blank" rel="noreferrer" aria-label="GitHub Profile" className="w-10 h-10 rounded-full flex items-center justify-center border border-gray-600 text-gray-400 hover:bg-emerald-500 hover:text-gray-900 hover:border-emerald-500 transition duration-300 transform hover:scale-110">
      <GitHubIcon />
    </a>
    <a href={links.kaggle} target="_blank" rel="noreferrer" aria-label="Kaggle Profile" className="w-10 h-10 rounded-full flex items-center justify-center border border-gray-600 text-gray-400 hover:bg-emerald-500 hover:text-gray-900 hover:border-emerald-500 transition duration-300 transform hover:scale-110">
      <KaggleIcon />
    </a>
  </div>
);
...
const Typewriter = ({ words }: { words: string[] }) => {
  ...
  return (
    <span className="text-emerald-500 inline-block text-start" dir="auto">
      {words[index].substring(0, subIndex)}
      <span className={`inline-block w-0.5 h-6 mx-1 bg-emerald-500 align-middle transition-opacity ${blink ? 'opacity-100' : 'opacity-0'}`}></span>
    </span>
  );
};
    ...
    <div className={`min-h-screen flex flex-col ${fontClass} overflow-x-hidden bg-gray-900 text-gray-100 relative pt-20 transition-all duration-300 antialiased`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <main className="flex-grow z-10">
        ...
        <section id="about" className="py-20 bg-gray-900/50">
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
              <h2 className="text-3xl md:text-4xl font-bold text-gray-100 text-center mb-10">
                <span className="border-b-4 border-emerald-500 pb-2">{content.about.title}</span>
              </h2>
        ...
        <section id="services" className="py-20">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <h2 className="text-3xl md:text-4xl font-bold text-gray-100 text-center mb-16">
                 <span className="border-b-4 border-emerald-500 pb-2">{content.services.title}</span>
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                {content.services.items.map((service, idx) => (
                  <Reveal key={idx} className="h-full">
                     <Tilt3D className="bg-gray-800/60 backdrop-blur rounded-xl p-8 border border-gray-700 hover:border-emerald-500 hover:bg-gray-800 transition-all duration-300 group h-full text-start">
                        <div className="w-14 h-14 bg-gray-900 rounded-lg flex items-center justify-center mb-6 group-hover:scale-110 transition duration-300 border border-gray-700 group-hover:border-emerald-500 rtl:scale-x-[-1]">
                            <svg className="w-8 h-8 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={service.iconPath}></path>
                            </svg>
                        </div>
                        <h3 className="text-xl font-bold text-gray-100 mb-3 group-hover:text-emerald-500 transition">{service.title}</h3>
                 ...
                 <Reveal key={idx} className="relative flex items-center justify-center w-full group">
                   ...
                   <div className={`w-full md:w-1/2 flex flex-col relative pl-12 rtl:pl-0 rtl:pr-12 md:pl-0 rtl:md:pr-0 ${idx % 2 === 0 ? 'md:mr-auto md:ml-0 md:pr-16 md:pl-0 md:items-end rtl:md:ml-auto rtl:md:mr-0 rtl:md:pl-16 rtl:md:pr-0' : 'md:ml-auto md:mr-0 md:pl-16 md:pr-0 md:items-start rtl:md:mr-auto rtl:md:ml-0 rtl:md:pr-16 rtl:md:pl-0'}`}>
                      <Tilt3D className="w-full p-6 bg-gray-800 rounded-xl border border-gray-700 hover:border-emerald-500 transition shadow-lg text-start">
                         ...
                         <p className="text-sm text-emerald-400 font-semibold mb-3">{item.company}</p>
                         <ul className="list-disc list-inside text-sm text-gray-400 space-y-1 marker:text-emerald-500">
        ...
        <section id="projects" className="py-20">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
               <h2 className="text-3xl md:text-4xl font-bold text-gray-100 text-center mb-16">
                  <span className="border-b-4 border-emerald-500 pb-2">{content.projects.title}</span>
              </h2>
                  ...
                  <Reveal key={idx} className="h-full">
                     <Tilt3D className="bg-gray-800 rounded-2xl border border-gray-700 overflow-hidden hover:shadow-[0_10px_30px_-10px_rgba(16,185,129,0.3)] hover:scale-105 hover:border-emerald-500/50 hover:z-10 relative transition-all duration-300 flex flex-col group h-full text-start">
                        ...
                        <div className="p-6 flex flex-col flex-grow relative">
                          <h3 className="text-2xl font-bold text-gray-100 mb-2 group-hover:text-emerald-500 transition">{project.title}</h3>
             ...
             <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                   {content.certifications.items.map((cert, idx) => (
                     <Reveal key={idx} className="h-full">
                        <Tilt3D className="bg-gray-800 rounded-lg p-6 border border-gray-700 hover:border-emerald-500 hover:shadow-lg transition flex flex-col h-full group text-start">
                           <div className="mb-4 text-emerald-500 rtl:scale-x-[-1]">
                           ...
                           <h4 className="font-bold text-gray-100 mb-2 group-hover:text-emerald-500 transition line-clamp-2">{cert.title}</h4>
                           <p className="text-sm text-gray-400 mb-1">{cert.issuer}</p>
                           <p className="text-xs text-gray-500 mb-4">{cert.date}</p>
                           <a href={cert.link} target="_blank" rel="noreferrer" className="mt-auto text-xs font-semibold text-emerald-500 hover:text-gray-100 flex items-center gap-1 transition">
                ...
                <div className="text-center mt-12">
                    <a href={links.certificationsDrive} target="_blank" rel="noreferrer" className="inline-flex items-center px-6 py-3 rounded-full border border-gray-600 text-gray-300 hover:border-emerald-500 hover:text-emerald-500 transition-colors font-semibold">
                        ...
                        <span className="ml-2 rtl:mr-2 rtl:ml-0"><ExternalLink /></span>
                    </a>
                </div>
             </div>
        </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 text-center md:text-start">
          ...
          <div className="space-y-6">
             ...
             <ul className="text-gray-400 space-y-3 text-sm font-medium">
                {content.nav.map(item => (
                    <li key={item.id}><a href={`#${item.id}`} onClick={(e) => scrollToSection(e, item.id)} className="hover:text-emerald-500 transition-colors duration-200 block w-fit mx-auto md:mx-0 rtl:md:ml-auto rtl:md:mr-0">{item.label}</a></li>
                ))}
             </ul>
          </div>
          <div className="space-y-6">
            ...
            <ul className="text-gray-400 space-y-4 text-sm">
                <li className="flex items-start justify-center md:justify-start gap-3 group">
                    <div className="p-2 bg-gray-800 rounded-lg text-emerald-500 group-hover:bg-emerald-500 group-hover:text-gray-900 transition mt-[-4px]">
